"""
3D Microstructural Dataset Generator for SOFC Electrode Modeling
...
class SOFCMicrostructureGenerator:
    ...
    def save_vtk(self, filename: str):
        """Save microstructure as VTK file."""
        print(f"Saving VTK file {filename}...")
        
        # Convert to VTK format
        vtk_data = numpy_support.numpy_to_vtk(
            self.microstructure.flatten(), deep=True, array_type=vtk.VTK_UNSIGNED_CHAR
        )
        ...
        grid.SetDimensions(self.resolution)
    ...
    def visualize_3d(self, save_path: Optional[str] = None):
        """Create 3D visualization of the microstructure."""
        ...
        phase_colors = {
            self.PORE: 'white',
            self.NI: 'gold',
            self.YSZ_ANODE: 'lightblue',
            self.YSZ_ELECTROLYTE: 'darkblue',
            self.INTERLAYER: 'red'
        }
        
        # Add each phase as separate mesh
        for phase_id, color in phase_colors.items():
            phase_mask = self.microstructure == phase_id
            if np.any(phase_mask):
                phase_grid = grid.extract_cells(phase_mask.ravel())
                if phase_grid.n_cells > 0:
                    plotter.add_mesh(phase_grid, color=color, opacity=0.7, 
    ...
    def generate_analysis_report(self) -> pd.DataFrame:
        ...
        interface_areas = self._calculate_interface_areas()
        ...
        for phase, props in self.phase_properties['phases'].items():
            report_data.append({
                ...
                'Volume_um3': props['volume_um3'],
                'Count': props['count']
            })
        
        # Add interface information
        for interface, area in interface_areas.items():
            report_data.append({
                'Phase': f'Interface_{interface}',
                'Volume_Fraction': 0,
                'Volume_um3': 0,
                'Count': area
            })
    ...
    def _calculate_interface_areas(self) -> Dict[str, float]:
        ...
        interfaces['Anode_Electrolyte'] = np.sum(interface_mask) * (self.voxel_size ** 2)
...
def main():
    ...
    report.to_csv('output/phase_analysis.csv', index=False)
    
    # Create visualization
    generator.visualize_3d('output/microstructure_3d.png')
    ...
    print("  - sofc_microstructure.vtk (VTK format)")
    print("  - phase_analysis.csv (Analysis report)")
    print("  - microstructure_3d.png (3D visualization)")
