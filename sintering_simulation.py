...
class WarpageAnalysis:
    ...
    def calculate_warpage(self, thermal_stress: np.ndarray, 
        ...
        # Temperature-dependent warpage contribution
        ...
        # Higher stress gradients and lower creep relaxation → more warpage
        base_warpage = 50  # Base warpage in µm
        stress_contribution = stress_ratio * 200  # Up to 200 µm from stress
        gradient_contribution = stress_gradient / 1e6 * 100  # Up to 100 µm from gradients
        creep_benefit = creep_strain[-1] * 1000  # Creep reduces warpage
        
        warpage = base_warpage + stress_contribution + gradient_contribution - creep_benefit
...
class SinteringSimulator:
    ...
    def simulate_profile(self, profile: SinteringProfile, 
        ...
        # Calculate warpage
        final_warpage = self.warpage.calculate_warpage(thermal_stress, creep_strain)
        
        # Calculate residual strain (Lagrangian strain) with realistic scaling
        # Higher temperatures and longer times → more creep → higher residual strain
...
def create_professional_plot(simulator: SinteringSimulator, 
    ...
    max_warpage = np.max(warpages) * 0.8
    max_strain = np.max(strains) * 0.7
