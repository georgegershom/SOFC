...
def main():
    print("="*60)
    print("SOFC MICROSTRUCTURE DATASET GENERATION")
    print("="*60)
    
    # Parameters - using smaller resolution for reliability
    resolution = (128, 128, 64)  # Reduced size
    ...
    n_pores = int(porosity * np.prod(resolution) / 1000)  # Reduced for efficiency
    ...
    # Create Ni particles
    n_ni_particles = int(ni_ysz_ratio * np.sum(solid_mask) / 500)  # Reduced for efficiency
    ...
    for i in range(n_ni_particles):
        ...
        if len(solid_indices[0]) > 0:
            ...
            # Create Ni particle
            radius = np.random.uniform(1, 2.5)
            x, y, z = np.ogrid[:resolution[0], :resolution[1], :resolution[2]]
            dist = np.sqrt((x - center[0])**2 + (y - center[1])**2 + (z - center[2])**2)
    ...
    if start_z >= 0:
        ...
        for z in range(start_z, resolution[2]):
            height_variation = np.random.normal(0, 0.2, (resolution[0], resolution[1]))
    ...
    # Save HDF5
    print("\nSaving data...")
    h5_filename = 'output/sofc_microstructure.h5'
    ...
    for z in range(microstructure.shape[2]):
        ...
        tiff_filename = f'output/sofc_microstructure_z{z:03d}.tif'
    ...
    plt.tight_layout()
    plt.savefig('output/sofc_microstructure_slices.png', dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig('output/sofc_microstructure_phase_distribution.png', dpi=300, bbox_inches='tight')
    plt.close()
