...
class WeldingDatasetGenerator:
    ...
    def generate_tier1_experimental_data(self, n_samples=300):
        ...
        material_combinations = np.random.choice(
            list(self.parameter_ranges['material_combination'].values()),
            size=n_samples
        )
    ...
    def generate_tier2_simulation_data(self, n_samples=10000):
        ...
        input_data = np.random.uniform(
            ...
            high=[self.parameter_ranges[param][1] for param in self.parameter_ranges.keys() 
                  if param != 'material_combination'],
            size=(n_samples, len(self.parameter_ranges) - 1)
        )
    ...
    def _generate_physics_based_outputs(self, input_data, param_names, fidelity='high'):
        ...
        for i in range(n_samples):
            ...
            outputs = [
                max(0.1, nugget_width + np.random.normal(0, noise_factor * nugget_width)),
                max(0.05, penetration_depth + np.random.normal(0, noise_factor * penetration_depth)),
                max(0.1, haz_width + np.random.normal(0, noise_factor * haz_width)),
                crack_presence,
                max(0, min(15, porosity + np.random.normal(0, noise_factor * porosity))),
                max(0, undercut + np.random.normal(0, noise_factor * undercut)),
                max(100, tensile_strength + np.random.normal(0, noise_factor * tensile_strength)),
                max(10, peel_strength + np.random.normal(0, noise_factor * peel_strength)),
                max(1, contact_resistance + np.random.normal(0, noise_factor * contact_resistance)),
                max(50, cycles_to_failure + np.random.normal(0, noise_factor * cycles_to_failure)),
                max(0, min(80, strength_degradation + np.random.normal(0, noise_factor * strength_degradation))),
                max(0, resistance_increase + np.random.normal(0, noise_factor * resistance_increase)),
                max(0.1, imc_thickness + np.random.normal(0, noise_factor * imc_thickness)),
                max(1, creep_time + np.random.normal(0, noise_factor * creep_time))
            ]
    ...
    def generate_dataset_report(self, master_dataset, ml_data):
        ...
        report = {
            ...
            'correlation_analysis': master_dataset.corr(),
