#!/usr/bin/env python3
"""
SOFC Thermal History Data Generator - Optimized Version
Generates comprehensive thermal data for SOFC analysis with memory optimization
"""
...
class SOFCThermalDataGeneratorOptimized:
    def __init__(self):
        ...
        # Reduced spatial grid for memory efficiency
        self.x = np.linspace(0, self.cell_length, 20)  # Reduced from 50
        self.y = np.linspace(0, self.cell_width, 20)   # Reduced from 50
        self.z_anode = np.linspace(0, self.anode_thickness, 5)  # Reduced from 10
        self.z_electrolyte = np.linspace(self.anode_thickness, 
                                       self.anode_thickness + self.electrolyte_thickness, 3)  # Reduced from 5
        self.z_cathode = np.linspace(self.anode_thickness + self.electrolyte_thickness,
                                   self.anode_thickness + self.electrolyte_thickness + self.cathode_thickness, 5)  # Reduced from 10
    ...
    def generate_sintering_data(self):
        ...
        # Reduced time profile for sintering process
        time_sintering = np.linspace(0, 8*3600, 200)  # Reduced from 1000 to 200
        ...
        # Generate data for each time step (reduced sampling)
        sintering_data = []
        for i, t in enumerate(time_sintering[::4]):  # Sample every 4th point
    ...
    def generate_thermal_cycling_data(self):
        ...
        for cycle in range(num_cycles):
            ...
            # Add spatial variations (reduced sampling)
            for i, (t, temp) in enumerate(zip(cycle_time[::2], cycle_temp[::2])):  # Sample every 2nd point
    ...
    def calculate_residual_stresses(self, sintering_data):
        ...
        sample_data = sintering_data[::10]  # Sample every 10th point
    ...
    def generate_all_data(self):
        ...
        summary = {
            'sintering_data': {
                'total_records': int(len(sintering_data)),
                'temperature_range': [float(sintering_data['temperature'].min()), float(sintering_data['temperature'].max())],
                'time_range': [float(sintering_data['time'].min()), float(sintering_data['time'].max())]
            },
            'cycling_data': {
                'total_records': int(len(cycling_data)),
                'num_cycles': int(cycling_data['cycle_number'].max()),
                'temperature_range': [float(cycling_data['temperature'].min()), float(cycling_data['temperature'].max())]
            },
            'steady_state_data': {
                'total_records': int(len(steady_state_data)),
                'temperature_range': [float(steady_state_data['temperature'].min()), float(steady_state_data['temperature'].max())],
                'layers': steady_state_data['layer'].unique().tolist()
            },
            'stress_data': {
                'total_records': int(len(stress_data)),
                'stress_range': [float(stress_data['thermal_stress'].min()), float(stress_data['thermal_stress'].max())],
                'strain_range': [float(stress_data['thermal_strain'].min()), float(stress_data['thermal_strain'].max())]
            }
        }
...
if __name__ == "__main__":
    generator = SOFCThermalDataGeneratorOptimized()
